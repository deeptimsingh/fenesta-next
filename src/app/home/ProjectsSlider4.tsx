...
export default function ProjectsSlider() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // gsap.set registers the start values GSAP will tween FROM.
      // These must match the inline styles on the element exactly so GSAP's
      // first tick is a no-op (no flash, no jump).
      gsap.set(slider, {
        width:        "50vw",
        left:         "50%",
        xPercent:     -50,
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub:    true,  // 1:1 with scroll — no lag, no jerk
          invalidateOnRefresh: true,  // recalc on resize without flicker
  ...
  return (
    ...
    >
      ...
      {/*
        * Initial state baked into inline style — prevents the full-width flash
        * that occurs in the gap between React render and gsap.set() running.
        * GSAP overwrites these on its first tick; values match so no jump.
        */}
      <div
        ref={sliderInnerRef}
