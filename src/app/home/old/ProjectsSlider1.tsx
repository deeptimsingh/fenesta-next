...
export default function ProjectsSlider() {
  ...
  useEffect(() => {
    ...
    if (!el || !slider || !thumbsContainer || !thumbsWrapper) return;
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(thumbsContainer, { opacity: 0, y: 100, pointerEvents: "none" });
      gsap.set(thumbsWrapper,   { xPercent: -10 });
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          scrub:         1.2,
          pin:           el,           // pin the outer wrapper (not the slider inner)
          pinSpacing:    true,
          anticipatePin: 1,

          onUpdate(self) {
            // Enable interactivity only when fully expanded; reverse on scroll-back
            setSliderEnabled(self.progress > 0.95);
            // Also toggle pointer-events on thumbs container directly
            if (thumbsContainer) {
              thumbsContainer.style.pointerEvents = self.progress > 0.95 ? "auto" : "none";
            }
          },
        },
      });

      // Step 1 — expand to full width
      tl.to(slider, {
        width:        "100vw",
        left:         "0%",
        xPercent:     0,
      ...
      // Step 3 — thumbnails
      tl.to(thumbsContainer, { opacity: 1, y: 0, duration: 0.4 },        0.85);
      tl.to(thumbsWrapper,   { xPercent: 0,       duration: 0.4 },        0.85);
  ...
  return (
    <div
      ref={containerRef}
      className="product-wrap relative w-full h-[160vw] md:h-screen overflow-hidden bg-white text-black"
    >
      {/* slider-inner: GSAP controls its width/position */}
      <div ref={sliderInnerRef} className="slider-inner h-full">
