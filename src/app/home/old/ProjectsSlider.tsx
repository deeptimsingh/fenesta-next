...
export default function ProjectsSlider() {
  ...
  const [sliderEnabled, setSliderEnabled] = useState(false); // enable interactions after scroll reveal
...
useHeadingAnimation({
  ...
  // Caption animation: show active slide caption, hide others
  function animateCaption(index: number) {
...
useEffect(() => {
  ...
  function buildThumbs(swiper: SwiperClass) {
  ...
  ------------------------------------------------------------ */
  function updateArrows(swiper: SwiperClass) {
  ...
  function syncVideoPlayback(swiper: SwiperClass) {
    if (!swiper?.el) return;
  ...
  function runSyncAfterTransition(swiper: SwiperClass) {
    requestAnimationFrame(() => {
  ...
  return (
    <div ref={containerRef} className="product-wrap relative w-full h-[160vw]  md:h-screen overflow-hidden bg-white text-black ">
      {/* Width is controlled by GSAP; don't force w-full here */}
