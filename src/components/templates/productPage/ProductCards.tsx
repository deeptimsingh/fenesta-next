...
export default function ProductCards() {
  ...
  return (
    <section className="product-list relative common-padding">
        <div className="container">
          <div className="product-list-content mx-auto ">  
            <div
              ref={containerRef}
              className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-[1vw] gap-y-5"
            >
              {/* WINDOWS */}
              <Card
                title="Windows"
...
}) 
{
    ...
    <div className="product-card group relative overflow-hidden rounded-2xl w-full cursor-pointer">
      <div className="relative h-[100vw] max-h-[357px] w-full overflow-hidden rounded-2xl bg-theme/40">
        ...
        {/* GRADIENT OVERLAY */}
        <div
          className="gradient-overlay absolute inset-0 z-10 pointer-events-none opacity-100 bg-gradient-to-t from-black/70 via-black/25 to-transparent"
        />
        ...
        <div className="absolute inset-0 z-20 flex flex-col justify-end text-white h-min-content">
          <div className="p-4 pb-0">
            ...
            <div className="hover-content mb-2 opacity-0 translate-y-2 pointer-events-none group-hover:pointer-events-auto group-hover:opacity-100 group-hover:translate-y-0 group-focus-within:pointer-events-auto group-focus-within:opacity-100 group-focus-within:translate-y-0 transition-[opacity,transform] duration-300 ease-out">
              {links.map((link) => (
                <Link
            ...
            <div className="title-button-container flex items-center justify-between bg-theme/50 group-hover:bg-theme/0  py-2 -mx-4 px-4 transition-all">
              ...
              <Link href={`/product-page?category=${encodeURIComponent(title.toLowerCase())}`} className="arrow-btn w-10 h-10 rounded-full bg-[#0288D1] flex items-center justify-center hover:bg-[#039BE5] opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-all duration-500 ease-out">
